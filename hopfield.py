...
class Hopfield:
    ...
    def append_practice_info(self, image_directory, images=None):
    ...
    def predict(self, input_image, name_directory):
        ...
        self.iter = 150
        self.async_iter = 1500
        try:
            if self.iter < 0 and self.async_iter < 0:
    ...
    def calc_predict(self, input_image, index_iter=0): #, iter, async_iter):
        self.iter = 150
        self.async_iter = 1500
        energy_condition, status_el = self.energy_condition(input_image.flatten()), input_image.flatten()
        #for index_iter in range(self.iter):
        while index_iter in range(self.iter):
            #while column_iter in range(self.async_iter):
            for _ in range(self.async_iter):
